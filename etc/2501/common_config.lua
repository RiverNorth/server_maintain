...
low_netspeed_check_tick		= 30000			-- 低网络延迟客户端与GS的心跳时间(毫秒)
normal_netspeed_check_tick	= 60000			-- 非网络延迟客户端与GS的心跳时间(毫秒)
max_no_msg_duration			= 600000		-- 服务器与客户端保持连接的最长时间(毫秒)

open_public_log_db            = false                -- 是否开启公用日志库（字段参数：false、true）

public_log_db_ip = "192.168.201.38"                      -- 公用日志数据库的IP
public_log_db_port = 3306                           -- 公用日志数据库的端口
public_log_db_name = "public_log"                   -- 公用日志数据库的名称
public_log_db_cs = "UTF-8"                          -- 公用日志数据库字符集

-------------------------------------------------------------------------------------------------
current_min_version			= 1			-- 最小版本号
current_max_version			= 500			-- 最大版本号
current_area_id		= 141         -- 当前区号

register_account   = true  -- 是否开启账号注册功能(非官方服务器必须为false)
activate_account   = true  -- 是否开启账号激活功能(非官方服务器必须为false)


current_gift_type       = 3           ----礼包类型（1-oppo/VIVO，2-PP）
server_name        = "IOS官方服-2501"   -------用于telnet服务器时显示服务器名称（运维同事根据服务器名称配置）

current_area_type            =  2                -- 分渠道区类型（区分充值定额及VIP等级1-安卓和越狱IOS---2-IOS官方）
